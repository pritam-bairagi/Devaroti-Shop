...
const USER_LEVELS = {
  ...
  Bronze: {
    ...
    minSpent: 5000,
    maxSpent: 9999,
    benefits: ['Priority Support', '5% Cashback', 'Free Delivery']
  },
  Silver: {
    name: 'Silver',
    color: '#C0C0C0',
    ...
    minSpent: 10000,
    maxSpent: 49999,
    benefits: ['VIP Support', '10% Cashback', 'Free Delivery', 'Exclusive Offers']
  },
  Gold: {
    name: 'Gold',
    color: '#FFD700',
    ...
    minSpent: 50000,
    maxSpent: 99999,
    benefits: ['24/7 Premium Support', '15% Cashback', 'Free Express Delivery', 'Early Access']
  },
  Platinum: {
    name: 'Platinum',
    color: '#E5E4E2',
    ...
    minSpent: 100000,
    maxSpent: 499999,
    benefits: ['Dedicated Account Manager', '20% Cashback', 'Free Express Delivery', 'Birthday Gifts']
  },
  VIP: {
    name: 'VIP',
    color: '#8B4513',
    ...
    minSpent: 500000,
    maxSpent: Infinity,
    benefits: ['Personal Shopper', '25% Cashback', 'Free Premium Delivery', 'Invite-only Events']
  }
};
...
const Dashboard = () => {
    ...
    try {
      if (editingAddress) {
        const updatedAddresses = addresses.map(addr => 
          (addr._id || addr.id) === (editingAddress._id || editingAddress.id) ? addressData : addr
        );
  ...
  const handleSetDefaultAddress = async (addressId) => {
    ...
    setAddresses(updatedAddresses);
    const newDefault = updatedAddresses.find(addr => (addr._id || addr.id) === addressId);
  ...
  const userLevel = user?.level || 'Plastic';
  const levelConfig = USER_LEVELS[userLevel] || USER_LEVELS.Plastic;
