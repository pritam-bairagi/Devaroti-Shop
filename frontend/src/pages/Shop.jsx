...
// ==================== PRODUCT DETAIL PAGE ====================
function ProductDetailPage({ product, allProducts, onAddCart, onWishlist, wishlisted, onBack, user, setUser }) {
    ...
    <div className="min-h-screen bg-gray-50 pb-10">
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
          {/* Images Section - With thumbnail navigation */}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <p className="font-bold text-gray-800">{product.sellerName || 'Official Store'}</p>
...
function RelatedProductCard({ product, onAddCart, onWishlist, wishlisted }) {
  const discount = product.oldPrice ? Math.round((1 - product.price / product.oldPrice) * 100) : null;
...
function ProductCard({ product, view, onAddCart, onWishlist, wishlisted, onProductClick, user, setUser }) {
  ...
  return (
    <div
      ...
      className="bg-white rounded-xl border border-gray-200 hover:shadow-xl transition-all overflow-hidden relative group"
    >
      {/* Badges */}
          ...
          <main className="max-w-7xl mx-auto px-4 py-6">
            ...
            <div className="mt-10 grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                ...
                <div key={s.title} className="bg-white border border-gray-200 rounded-xl p-4 flex items-center gap-3 hover:shadow-md transition group">
                  <span className="text-orange-400 group-hover:scale-110 transition">{s.icon}</span>
                  <div>
              ...
              <div className="p-6">
                ...
                <div className="space-y-1">
                  <Link
                    to="/"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="flex items-center gap-3 p-3 rounded-xl hover:bg-orange-50 transition text-gray-700"
                  >
                    <Home size={20} />
                  ...
                  <Link
                    to="/shop"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="flex items-center gap-3 p-3 rounded-xl bg-orange-50 text-orange-600"
                  >
                    <ShoppingBag size={20} />
                  ...
                  <Link
                    to="/favorites"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="flex items-center justify-between p-3 rounded-xl hover:bg-orange-50 transition"
                  >
                    <div className="flex items-center gap-3">
                      <Heart size={20} />
                  ...
                  {NAV_LINKS.map(link => (
                    <Link
                      ...
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="flex items-center gap-3 p-3 rounded-xl hover:bg-orange-50 transition text-sm"
                    >
                      {link === 'Flash Sale' && <Zap size={16} className="text-orange-500" />}
                      {link === 'Discounts' && <Percent size={16} className="text-green-500" />}
                      {link === 'Hot Offers' && <Flame size={16} className="text-red-500" />}
                      {link === 'GiveWays' && <Gift size={16} className="text-purple-500" />}
                      <span>{link}</span>
                    </Link>
                  ))}
                  ...
                  {user ? (
                    <>
                      ...
                      {user.role === 'admin' && (
                        <Link
                          to="/admin"
                          onClick={() => setIsMobileMenuOpen(false)}
                          className="flex items-center gap-3 p-3 rounded-xl hover:bg-purple-50 transition text-purple-600"
                        >
                          <Settings size={20} />
                      ...
                      {user.role === 'seller' && (
                        <Link
                          to="/seller"
                          onClick={() => setIsMobileMenuOpen(false)}
                          className="flex items-center gap-3 p-3 rounded-xl hover:bg-orange-50 transition text-orange-600"
                        >
                          <Award size={20} />
                    ...
                    <Link
                      to="/login"
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="block w-full p-3 bg-gradient-to-r from-orange-500 to-amber-500 text-white text-center font-bold rounded-lg hover:from-orange-600 hover:to-amber-600 transition"
                    >
                      Log In / Register
                    </Link>
                  )}
